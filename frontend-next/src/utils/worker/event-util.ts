import type { Result } from '@/utils/worker/worker-thread.ts'

/**
 * 事件处理器
 *
 *  - type 事件名称
 *  - processor 处理钩子（发送的事件如果携带数据，会传一个data）
 */
export interface EventProcessor {
  type: string
  processor: (data: any) => void
}

/**
 * 接收工作线程的事件（主线程配置）
 *
 * @param eventProcessors 事件处理器
 * @param result 工作线程的状态结果（Promise）
 */
export function receiveWorkerEvent(
  eventProcessors: EventProcessor[],
  result: Result,
):
(event: MessageEvent) => void {
  return (event: MessageEvent) => {
    // 处理正常结束事件
    if (event.data.type === 'returnToMain' && event.data.endFlag) {
      if (result.resolve) {
        result.resolve(event.data.data)
      }
      else {
        throw new Error('未启动的线程，不可以提前返回和退出！')
      }
      return
    }
    // 处理异常结束事件
    if (event.data.type === 'catchToMain' && event.data.endFlag) {
      if (result.reject) {
        result.reject(event.data.data)
      }
      else {
        throw new Error('未启动的线程，不可以提前返回和退出！')
      }
      return
    }
    // 处理自定义事件
    for (const processor of eventProcessors) {
      if (processor.type === event.data.type) {
        processor.processor(event.data.data)
        return
      }
    }
    console.warn('主线程接收到未声明处理的事件：', event)
  }
}

/**
 * 接收主线程的事件（工作线程配置）
 *
 * @param eventProcessors 事件处理器
 * @param main 入口函数
 */
export function receiveMainEvent(
  eventProcessors: EventProcessor[],
  main?: (data: any) => void,
):
(event: MessageEvent) => void {
  return (event: MessageEvent) => {
    // 处理启动事件
    if (event.data.type === 'startWorker' && event.data.startFlag) {
      if (main) {
        main(event.data.data)
      }
      else {
        throw new Error('无法启动线程，入口函数未定义！')
      }
      return
    }
    // 处理自定义事件
    for (const processor of eventProcessors) {
      if (processor.type === event.data.type) {
        processor.processor(event.data.data)
        return
      }
    }
    console.warn('工作线程接收到未声明处理的事件：', event)
  }
}

/**
 * 向主线程发送事件
 *
 * @param type 事件名称
 * @param data 数据
 * @param transfer 可转移数据
 */
export function postToMain(type: string, data: any, transfer?: Transferable[]): void {
  postMessage({
    type,
    data,
  }, transfer || [])
}

/**
 * 向主线程返回运行结果（发送结束事件）
 *
 * @param data 数据
 * @param transfer 可转移数据
 */
export function returnToMain(data: any, transfer?: Transferable[]): void {
  postMessage({
    type: 'returnToMain',
    endFlag: true,
    data,
  }, transfer || [])
}

/**
 * 向主线程报告异常退出（发送结束事件）
 *
 * @param error 异常数据
 * @param transfer 可转移数据
 */
export function catchToMain(error: any, transfer?: Transferable[]): void {
  postMessage({
    type: 'catchToMain',
    endFlag: true,
    data: error,
  }, transfer || [])
}

/**
 * 向工作线程发送事件
 *
 * @param to 工作线程
 * @param type 事件名称
 * @param data 数据
 * @param transfer 可转移数据
 */
export function postToWorker(to: Worker, type: string, data: any, transfer?: Transferable[]): void {
  to.postMessage({
    type,
    data,
  }, transfer || [])
}
