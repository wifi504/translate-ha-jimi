...
const STORAGE_KEY = 'encrypted_ha_jimi_contacts'
...
export const useContactStore = defineStore('contact', () => {
  ...
  async function setPassword(pass: string, confirm: string): Promise<ResultStatus> {
    ...
    if (exists) {
      if (!unlocked.value) return 'access_denied'
  ...
  async function auth(pass: string): Promise<ResultStatus> {
    ...
    try {
      ...
      const encryptedData = decode(payload.encryptedData)
      ...
      const obj: Record<string, string> = uint8ArrayToObject(new Uint8Array(decrypted))

      const map = new Map<string, Uint8Array>()
      for (const [k, v] of Object.entries(obj)) {
        map.set(k, hexToUint8Array(v))
      }
  ...
  async function save(): Promise<void> {
    ...
    const pwKey = await crypto.subtle.importKey(
      'raw',
      stringToUint8Array(password.value),
      { name: 'PBKDF2' },
      false,
      ['deriveKey'],
    )
    ...
    for (const [k, v] of contactMap.value.entries()) {
      obj[k] = uint8ArrayToHex(v)
    }

    const plaintext = objectToUint8Array(obj)
    const encrypted = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, aesKey, plaintext)

    const payload: EncryptedPayload = {
      version: 1,
      encryptedData: encode(new Uint8Array(encrypted), '哈基密文'),
      iv: encode(iv, '哈基向量'),
      salt: encode(salt, '哈基盐'),
      iterations,
    }
  ...
  async function setSecretKey(nickname: string, key: Uint8Array): Promise<ResultStatus> {
    if (!unlocked.value) return 'access_denied'
    if (contactMap.value.has(nickname)) return 'duplicate_user'
   ...
   * 获取当前联系人列表
   */
  const contactList: ComputedRef<string[]> = computed(() => {
  ...
  async function rename(oldName: string, newName: string): Promise<ResultStatus> {
    if (!unlocked.value) return 'access_denied'
    if (!contactMap.value.has(oldName)) return 'null_user'
  ...
  async function remove(nickname: string): Promise<ResultStatus> {
    if (!unlocked.value) return 'access_denied'
    if (!contactMap.value.has(nickname)) return 'null_user'
